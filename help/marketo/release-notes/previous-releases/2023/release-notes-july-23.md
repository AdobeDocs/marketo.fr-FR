...
Vous trouverez ci-dessous toutes les fonctionnalités incluses dans la version du 23 juillet. Vérifiez la disponibilité de vos fonctionnalités dans l’édition Adobe Marketo Engage.
...
<table> 
  ...
  <tr> 
   <td>Expédié</td>
   <td><a href="/help/marketo/product-docs/core-marketo-concepts/engagement-map/engagement-map-overview.md" target="_blank">Présentation de la carte des engagements</a></td>
  </tr>
  </tbody>
</table>
...
* **Activités Dynamic Chat améliorées** : les activités, filtres et déclencheurs de Dynamic Chat ont été améliorés avec des attributs supplémentaires et des conventions d’affectation de noms normalisées.
...
* **Désabonner les paramètres d’ajout pour les actions Sales Insight** : vous disposez désormais d’un meilleur contrôle sur vos messages de désabonnement avec de nouvelles options de configuration pour se désabonner de l’ajout. Les nouvelles fonctionnalités permettent aux administrateurs de contrôler quand, où et quels messages de désabonnement apparaîtront dans vos emails de vente.
...
* **Appels de champ dynamiques pour les actions d’aperçu des ventes** : vous pouvez désormais inviter votre équipe commerciale à personnaliser davantage des parties spécifiques d’un email avec de nouvelles invites de champ disponibles pour être ajoutées aux modèles de courrier électronique dans les actions d’aperçu des ventes.
...
<table> 
  ...
  <tr> 
   <td>Expédié</td>
   <td><a href="/help/marketo/product-docs/marketo-sales-insight/actions/templates/field-prompts.md" target="_blank">Invite de champ</a></td>
  </tr>
  </tbody>
</table>

* **Champs dynamiques par défaut pour les actions d’aperçu des ventes** : les champs dynamiques par défaut vous permettent de configurer une valeur de secours qu’un champ dynamique peut résoudre si aucune valeur n’est disponible pour être renseignée. Cela permet de s’assurer que les emails sont envoyés avec toute la personnalisation renseignée.
...
* **Prise en charge des Dynamic Chat pour Sales Insight** : des mises à jour ont été apportées pour prendre en charge les nouveaux attributs de Dynamic Chat introduits dans cette version pour tous les utilisateurs de Dynamic Chat.
...
* **Prise en charge Dynamic Chat des actions d’aperçu des ventes** : nous prenons désormais en charge les activités de Dynamic Chat dans le flux de piste, ainsi que les notifications lorsqu’une conversation en direct est envoyée à un agent.
...
* **Jetons de déclenchement Dynamic Chat** : présentation de nouveaux jetons de déclenchement pour les activités de Dynamic Chat à utiliser dans les moments intéressants.
...
* **Mise à niveau vers Google Analytics 4** : Marketo prenait auparavant en charge Universal Analytics, qui cessera d’enregistrer les données d’ici le 1er juillet 2023. Les utilisateurs RTP devront suivre le processus de migration dans leur tableau de bord Google Analytics pour terminer la prise en charge de l’intégration de Google Analytics 4 dans RTP. À l’avenir, les utilisateurs auront une balise alphanumérique unique (située dans la section `<head>` du code html) dans leurs pages d’entrée qui permet à Marketo d’envoyer des données aux Google Analytics.
