...
Créer et générer des campagnes par e-mail

Les sections marquées [FOR KEITH:] ont besoin d’un contenu plus spécifique au produit.\
Les espaces réservés aux captures d’écran utilisent [CAPTURE D’ÉCRAN : description] — extrayez des captures d’écran déjà dans le brouillon source ou capturez de nouvelles captures d’écran à partir de l’environnement d’évaluation.

Notez également Keith, j&#39;ai utilisé « Halo » comme espace réservé mais ce nom changera selon la décision de la direction.

Forme

[POUR KEITH : introduction de 1 à 2 phrases qui renforce la confiance et oriente le lecteur. Suggestion de structure : « D’ici la fin de ce guide, vous aurez créé et révisé votre première campagne complète par e-mail dans Halo. Du début à la fin, cela prend environ [TIME ESTIMATE].« ]
...
Un compte Halo actif (inscrivez-vous ici si vous ne l&#39;avez pas encore fait).

La configuration de votre marque sous Vos affaires > Marques. (En savoir plus)

(Facultatif mais recommandé) Un modèle d’e-mail HTML téléchargé sous Vos affaires > Modèles d’e-mail.

Un CSV d’audience prêt à être chargé. (Téléchargez un exemple de fichier CSV d’audience si vous avez besoin d’un point de départ.)
...
Demander les éléments essentiels
...
Sélectionnez un modèle de campagne préconfiguré dans la section des modèles située sous la barre d’invite.

Utilisez une option « m’aider à demander l’invite » dans la liste déroulante de la barre d’invite pour que Halo vous guide tout au long de la rédaction de votre invite.
...
Une invite Halo forte comprend :

L&#39;objectif de la campagne - ce que vous essayez d&#39;atteindre.

L’audience : à qui elle est destinée ou d’où proviennent les données de l’audience.
...
[FOR KEITH : présentation du chargement CSV. Couverture : emplacement du bouton de chargement dans l’interface utilisateur. - Colonnes obligatoires et attentes en matière de format. - Champs de personnalisation que Halo peut utiliser (prénom, date de dernière commande, catégorie de produit, etc.). - Lien vers l’exemple de fichier CSV. - Que se passe-t-il si les données sont désordonnées ou si les champs sont manquants ]

[CAPTURE D’ÉCRAN : flux de chargement CSV]

Conseil : excluez tous les contacts que vous ne souhaitez pas envoyer par e-mail (utilisateurs désabonnés, adresses internes, comptes de test) avant le chargement. Bien que nous activerons progressivement la fonctionnalité pour « exclure » des utilisateurs spécifiques ou « ajouter des attributs » au cours de l’essai, elle n’est pas immédiatement disponible à partir de la date de lancement.
...
Appuyez sur « Générer la campagne ». Halo :

Générer un plan de campagne structuré
...
Création dynamique du parcours en cours de route
...
C&#39;est là que l&#39;interface conversationnelle de Halo gagne sa vie. Pour apporter des changements, il suffit de parler à Halo :
...
« Remplacer l’attente de 3 jours par 5 jours »
...
Lorsque vous êtes satisfait(e) de la campagne, plusieurs options s’offrent à vous :

Lancez la campagne. (Actuellement indisponible avec notre version, mais sera activé dans les 2 semaines suivant notre première version, en attente d’une autorisation légale)
...
[FOR KEITH : détails sur chaque option et vérifications avant le lancement Halo s’exécute (conformité, liens rompus, champs de personnalisation manquants, etc.).]
...
Cas d’utilisation de notre collection : parcourez des exemples d’invites réels pour remixer vos propres campagnes.

Commencez avec Halo — Retour à l&#39;aperçu.

Demander Halo — Bonnes pratiques (bientôt disponible)
