---
description: Comment exporter un schéma de métadonnées d’objet personnalisé pour une utilisation avec l’API SOAP ou Munchkin.
title: Exportation de métadonnées d’objet personnalisé
exl-id: 4c23f07e-3491-43c5-8ea8-2e54c9a4b9af
feature: Custom Objects
TQID: https://experienceleague.adobe.com/7eZF1c45qtTibEHxI-QLu3KvT0rV-pRZSF66DxpD5eg
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2:
  - id: d1d0a9cd-295d-4976-8c39-ddae266f240e
topic_v2:
  - id: a004cc84-67b9-4a33-a3a7-8ec7273ef4dc
  - id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 115
ht-degree: 10%

---

# Exportation de métadonnées d’objet personnalisé {#custom-object-metadata-export}

Si vous utilisez l’API SOAP ou [!DNL Munchkin], le schéma de métadonnées d’objet personnalisé peut être exporté. Suivez les étapes ci-après.

>[!AVAILABILITY]
>
>Tous les utilisateurs de Marketo Engage n’ont pas acheté cette fonctionnalité. Pour plus d’informations, contactez l’équipe du compte Adobe (votre gestionnaire de compte).

1. Accédez à la zone **[!UICONTROL Admin]**.

   ![](assets/custom-object-metadata-export-1.png)

1. Cliquez sur **[!UICONTROL Objets personnalisés]**.

   ![](assets/custom-object-metadata-export-2.png)

1. Sélectionnez l’objet personnalisé Marketo à exporter.

   ![](assets/custom-object-metadata-export-3.png)

1. Cliquez sur le menu déroulant **[!UICONTROL Actions d’objet personnalisées]** et sélectionnez **[!UICONTROL Exporter l’objet]**.

   ![](assets/custom-object-metadata-export-4.png)

>[!NOTE]
>
>L’objet personnalisé doit être à l’état Approuvé pour être exporté.

Vous disposez désormais d’une feuille de calcul avec le schéma de l’objet personnalisé, dans trois onglets.

![](assets/custom-object-metadata-export-5.png)

![](assets/custom-object-metadata-export-6.png)
