...
Cela dépend des activités incluses dans le pipeline.
...
L’utilisation des activités vous indique le volume de marketing réalisé et vous aide à visualiser la croissance par rapport à identifié pour le niveau de produit pour lequel un contrat a été conclu. Les tableaux de bord peuvent également être utilisés comme guide pour déterminer le degré d’optimisation qui peut ou doit être réalisé en réduisant le nombre de champs mis à jour.
...
_Modifier la valeur des données_ est le type d’activité le plus courant. Indique le moment où une information sur un enregistrement de personne/prospect est mise à jour. Regroupez-les par champs qui sont modifiés le plus souvent afin de déterminer quelles informations sont utiles à vos opérations marketing et s’il existe des opportunités d’optimiser l’utilisation de la plateforme.

### Par campagne {#by-campaign}

Groupe par lequel les campagnes produisent le plus d’activités. Cela vous permet de voir si vous avez des campagnes particulièrement « bruyantes » qui créent plus d’activité que nécessaire. Découvrez les campagnes qui doivent être désactivées ou celles qui font plus de travail que prévu.
