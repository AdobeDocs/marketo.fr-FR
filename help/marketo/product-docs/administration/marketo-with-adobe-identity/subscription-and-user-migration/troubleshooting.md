---
description: Guide de dépannage pour les erreurs de migration des utilisateurs et utilisatrices IMS, telles que le caractère non valide de Gmail, l’utilisateur ou l’utilisatrice absent de l’annuaire, l’utilisateur ou utilisatrice inactif et le domaine non valide, avec des étapes de résolution.
...
**Cause principale** : si un compte fédéré (T3) est configuré pour l’organisation et qu’un compte individuel (T1) existe déjà pour l’utilisateur, toute tentative de création (le cas échéant) et de liaison d’un compte d’habilitation uniquement (T2e) au compte fédéré échoue avec le code d’erreur FAILED_TO_CREATE_TYPE2E_USER.

**Résolutions** :

1. Supprimez l’utilisateur individuel de l’organisation Adobe. _Remarque : l’utilisateur perdra l’accès à tous les produits et devra à nouveau y avoir droit ultérieurement. Si cela vous nuit, contactez l’assistance Adobe._
1. Réexécutez la migration des utilisateurs afin qu’un compte utilisateur fédéré soit créé pour cet utilisateur.
1. Le compte de droits uniquement est créé (le cas échéant) et lié au compte fédéré comme compte d’authentification.

**En savoir plus**
* [Configurer une identité sans authentification SSO](https://helpx.adobe.com/fr/enterprise/using/set-up-identity.html#:~:text=Set%20up%20identity%20without%20Single%20Sign%2DOn)
* [Configurer l&#39;identité avec l&#39;authentification SSO](https://helpx.adobe.com/fr/enterprise/using/set-up-identity.html#:~:text=Set%20up%20identity%20with%20Single%20Sign%2DOn)
