...
Cependant, **Marketo ne début pas automatiquement la collecte de ces données**. Pour que Marketo puisse capturer des valeurs de champs personnalisés en début, vous **devez** mapper ces champs personnalisés à un champ dans Marketo.
...
1. Cliquez sur dans la colonne **Champ Marketo**. Tapez pour rechercher le champ à mapper. Une fois que vous avez sélectionné un champ, cliquez sur **Enregistrer**.
   ...
   >[!NOTE]
   >
   >Si vous n’avez pas encore de champ dans Marketo pour mapper le champ Facebook à, découvrez comment [créer des champs personnalisés](/help/marketo/product-docs/administration/field-management/create-a-custom-field-in-marketo.md).
