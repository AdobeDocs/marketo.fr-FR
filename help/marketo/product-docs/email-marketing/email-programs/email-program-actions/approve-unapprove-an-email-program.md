---
unique-page-id: 2359462
description: Découvrez comment approuver ou annuler l’approbation d’un programme de messagerie. Terminez le workflow d’approbation afin que le programme puisse être planifié et envoyé.
title: Approuver/annuler l’approbation d’un programme d’e-mail
exl-id: 0bfb1306-7a98-4629-9264-63cfd5d6a690
feature: Email Programs
source-git-commit: 240b78561db11e169188698880d4707a5c1f64de
workflow-type: tm+mt
source-wordcount: '201'
ht-degree: 8%

---

# Approuver/annuler l’approbation d’un programme d’e-mail {#approve-unapprove-an-email-program}

>[!PREREQUISITES]
>
>* [Créer un programme de messagerie](/help/marketo/product-docs/email-marketing/email-programs/creating-an-email-program/create-an-email-program.md)
>* [Définir une audience avec une liste dynamique](/help/marketo/product-docs/email-marketing/email-programs/managing-people-in-email-programs/define-an-audience-with-a-smart-list.md) ou [Définir une audience en important une liste](/help/marketo/product-docs/email-marketing/email-programs/managing-people-in-email-programs/define-an-audience-by-importing-a-list.md)
>
>* [Choisir un e-mail existant](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/choose-an-existing-email.md) ou [Créer un e-mail pour un programme de messagerie](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/create-an-email-for-an-email-program.md)
>
>* [Planifier Votre Programme De Messagerie](/help/marketo/product-docs/email-marketing/email-programs/email-program-actions/schedule-your-email-program.md)

## Approuver votre programme de messagerie {#approve-your-email-program}

Maintenant que votre programme est prêt à être lancé, l’étape finale consiste à l’approuver. Voici comment procéder :

1. Accédez à **[!UICONTROL Activités marketing]**.

   ![](assets/login-marketing-activities-2.png)

   Sélectionnez votre programme de messagerie.
   ![](assets/selectemailprogram-2.jpg)

   >[!CAUTION]
   >
   >Si vous n’approuvez pas votre programme de messagerie, le message ne sera pas envoyé.

1. Sous la mosaïque **[!UICONTROL Approbation]**, continuez et cliquez sur **[!UICONTROL Approuver le programme]**.

   ![](assets/image2014-9-12-13-3a43-3a36.png)

1. Si nous détectons des problèmes, nous vous le faisons savoir. Corrigez-les et réessayez d’approuver.

   ![](assets/image2014-9-12-13-3a43-3a44.png)

   Parfait ! Vous avez terminé. Votre e-mail sera envoyé à l’heure prévue.

   ![](assets/image2014-9-12-13-3a43-3a56.png)

## Désapprobation de votre programme de messagerie {#unapprove-your-email-program}

Si vous changez d’avis et décidez de ne pas le publier, vous pouvez annuler l’approbation du programme de messagerie.

1. Accédez à **[!UICONTROL Activités marketing]**.

   ![](assets/login-marketing-activities-2.png)

1. Sélectionnez votre programme de messagerie.

   ![](assets/selectemailprogram-2.jpg)

1. Sous le volet **[!UICONTROL Approbation]**, cliquez simplement sur **[!UICONTROL Désapprouver le programme]**.

   ![](assets/image2014-9-12-13-3a44-3a28.png)

Terminé ! Votre e-mail ne sera pas envoyé tant que vous ne l’aurez pas à nouveau approuvé.
