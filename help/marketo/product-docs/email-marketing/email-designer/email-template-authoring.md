---
description: Découvrez comment créer des modèles d’e-mail autonomes pour les réutiliser. Créez des modèles personnalisés dans le Designer d’e-mail pour une conception et une cohérence plus rapides.
...
>[!NOTE]
>
>Les modèles d’e-mail dans le Designer d’e-mail peuvent uniquement être utilisés pour créer des e-mails dans le Designer d’e-mail. Ils ne peuvent pas être référencés dans l’éditeur d’e-mail classique.
...
>[!NOTE]
>
>Vous pouvez uniquement ajouter des images dans le Designer d’e-mail, aucun autre type de fichier pour le moment.
...
Les jetons fonctionnent dans le Designer d’e-mail de la même manière que dans l’éditeur classique, mais l’icône a un aspect différent. L’exemple ci-dessous décrit l’ajout d’un jeton de prénom avec un texte de remplacement.
...
1. Recherchez le jeton souhaité et cliquez sur l’icône **...** (cliquer sur l’icône + ajoute un jeton sans texte de remplacement).
   ...
   >[!NOTE]
   >
   >« Texte de secours » est le terme Designer d’e-mail pour la valeur par défaut. Exemple : ``{{lead.First Name:default=Friend}}``. Il est recommandé au cas où il n’y aurait aucune valeur pour la personne dans le champ que vous choisissez.
...
>[!MORELIKETHIS]
>
>[Création d’e-mails](/help/marketo/product-docs/email-marketing/email-designer/email-authoring.md){target="_blank"}
