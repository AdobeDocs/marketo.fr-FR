---
unique-page-id: 30081321
description: Comprendre les types de groupes dans Sales Connect. Découvrez Mes groupes, les groupes partagés et comment organiser des contacts.
title: Types de groupes
exl-id: 70b59c5d-f753-4b0d-b861-b2de379839a2
feature: Marketo Sales Connect
source-git-commit: 15427eacd2fc42a02f6a4c59d9102bacba02e57b
workflow-type: tm+mt
source-wordcount: '109'
ht-degree: 5%

---

# Types de groupes {#group-types}

Découvrez les différents types de groupes dans [!UICONTROL Sales Connect].

![](assets/one-7.png)

<table>
 <colgroup>
  <col>
  <col>
 </colgroup>
 <tbody>
  <tr>
   <th>Groupe</th>
   <th>Description</th>
  </tr>
  <tr>
   <td><p>[!UICONTROL All People]</p></td>
   <td>Tous les contacts de tous les utilisateurs qui sont visibles pour vous.</td>
  </tr>
  <tr>
   <td colspan="1"><p>[!UICONTROL Dynamic Groups]</p></td>
   <td colspan="1">Mes contacts : tous les contacts que vous possédez.<br>Désabonnements : contacts qui se sont désabonnés de la réception de la correspondance.</td>
  </tr>
  <tr>
   <td><p>[!UICONTROL My Groups]</p></td>
   <td>Groupes que vous avez créés. Ils peuvent contenir vos contacts ou les contacts qui ont été partagés avec vous.</td>
  </tr>
  <tr>
   <td><p>[!UICONTROL Team Groups]</p></td>
   <td>Groupes qui ont été partagés avec vous et/ou par vous. Ils peuvent contenir des contacts que possèdent vos coéquipiers ou des contacts que vous avez partagés avec eux.</td>
  </tr>
 </tbody>
</table>
