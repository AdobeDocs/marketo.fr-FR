...
**Que se passe-t-il** les utilisateurs n’ont pas accès en écriture aux champs personnalisés Tout nécessaires pour effectuer la transaction de journalisation de l’activité la plus récente ? L’équipe a peut-être installé le package mais n’a pas activé les champs corrects pour les utilisateurs.
