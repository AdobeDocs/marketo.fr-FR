---
unique-page-id: 2359516
description: Découvrez comment annuler l’approbation d’un fragment de code pour le modifier ou le supprimer. Supprimez les associations de ressources, puis utilisez les actions Extrait de code pour annuler l’approbation et définir sur le statut de brouillon.
title: Annuler l’approbation d’un extrait
exl-id: e3797788-704c-461f-a7b0-dac39eeaa1a7
feature: Snippets
TQID: https://experienceleague.adobe.com/AoSHjts1OJmIB-IB31eAYRZ8VG8kpQlw76vlJK3GRYA
product_v2: id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2: id: d65b4a73-87a3-4d56-b638-74e74d9939ceid: ed6be6bb-75bb-4ea9-9a42-3bcaa65e1bcc
subfeature_v2: id: a1d50dda-6d94-4e16-8c30-5eb7181c4650id: df8eb12b-4f82-491f-acbb-d74012ca5654
topic_v2: id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 97
ht-degree: 6%

---

# Annuler l’approbation d’un extrait {#unapprove-a-snippet}

Un fragment de code non approuvé ne peut pas être utilisé dans les e-mails ou les landing pages.

1. Accédez au **[!UICONTROL Design Studio]**.

   ![](assets/image2014-9-16-10-3a41-3a18.png)

1. Accédez à votre fragment de code et assurez-vous qu’il n’est **[!UICONTROL utilisé par]** aucune ressource.

   ![](assets/image2014-9-16-10-3a41-3a27.png)

   Si votre fragment de code est utilisé par d’autres ressources, supprimez ces associations avant de continuer.

1. Dans **[!UICONTROL Actions relatives aux fragments de code]**, cliquez sur **[!UICONTROL Désapprouver]**.

   ![](assets/image2014-9-16-10-3a41-3a54.png)

Votre fragment de code est maintenant en état de brouillon. Vous pouvez donc le modifier ou le supprimer.
